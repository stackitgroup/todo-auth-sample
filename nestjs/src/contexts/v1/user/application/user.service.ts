...
export class UserService {
  ...
  async signUp(
    ...
    const alreadyExists = (await this.userRepository.findByCondition({condition: {username: data.username}}))[0]
    ...
    const {
      accessToken,
      refreshToken
    } = await this.createTokens(data.userAgent, createdUser.id)
  ...
  async login(data: AccessDTO &  {userAgent: string}): Promise<User & RefreshResponse | null> {
    const found : User = (await this.userRepository.findByCondition({condition: {username: data.username}}))[0]
  ...
  async refreshTokens(
    ...
    try {
      const haveSameUserAgent : boolean = await this.haveSameUserAgent(userAgentId, userId)
      const haveSameRefreshToken : boolean = await this.haveSameRefreshToken(refreshToken, userId)

      if(haveSameUserAgent || haveSameRefreshToken) {
        throw new UnauthorizedException(`Wrong credentials. Provided: ${JSON.stringify({userAgentId, refreshToken})}`)
      }
      ...
      await this.userRepository.update(userId, {refreshToken: newRefreshToken, userAgent: userAgentId})
  ...
  async createTokens(userAgentId: string, userId: string): Promise<RefreshResponse> {
    try {
      const newRefreshToken = await this.generateRefreshToken({
        userAgentId,
        userId
  ...
  async haveSameRefreshToken(refreshToken: string, userId: string): Promise<boolean> {
    const user = await this.userRepository.findById(userId)
